...
export default function Button({
  ...
  const fullClassName = `${
    heightClass || "h-10"
  } inline-flex items-center py-1 px-4 focus:outline-none rounded-md ${classes} ${
    className || ""
  }`;
  return href ? (
    <AsLink className={fullClassName} href={href} target={target}>
      {children}
    </AsLink>
  ) : (
    <AsBtn onClick={onClick} className={fullClassName} type={type}>
      {children}
    </AsBtn>
  );
}
